...
- [x] Task 9: Fix Lint Errors and Virtual File System Bugs
...
- [x] Task 10: Fix Virtual File System Path and Command Parsing Bugs
  - [x] SubTask 10.1: Fix `mkdir` and path resolution to handle `..` correctly so `mkdir a/../b` creates `b` in the current directory, not `..` inside `a`.
  - [x] SubTask 10.2: Fix `Terminal` command parsing to handle quoted strings (e.g. `echo "hello" > "file with spaces.txt"`).
  - [x] SubTask 10.3: Fix `FileExplorer` and `TextEditor` file creation so that creating a file like `a/b.txt` either creates the parent directories or rejects the invalid filename, preventing orphaned nodes.
