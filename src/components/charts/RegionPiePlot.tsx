...
export default function RegionPiePlot({
  provinces,
  values,
  isDark,
  selectedProvince,
  ...
  return (
    <div className="relative isolate overflow-hidden rounded-xl" style={{ height }}>
      <Plot
        data={[
          {
            type: "pie",
            ...
            sort: false,
            direction: "clockwise",
            pull: selectedProvince ? provinces.map(p => (p === selectedProvince ? 0.08 : 0)) : undefined,
        ...
        onClick={evt => {
          ...
          if (typeof label === "string" && onSelectProvince) onSelectProvince(label);
        }}
      />
    </div>
  );
